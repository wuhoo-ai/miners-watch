...
namespace MinersWatch.Editor
{
    ...
    public static class CaveSceneAuthor
    {
        struct CaveDef
        {
            ...
            public string Title;
            public string MeshName => Depth switch // matches CaveBackgroundGenerator DEPTHS
            {
                DepthLevel.Shallow => "Shallow",
                DepthLevel.Medium => "Medium",
        ...
        static void AuthorOne(CaveDef def)
        {
            ...
            // Cave 3D background mesh (T014, generated by CaveBackgroundGenerator)
            BuildCave3DBackground(def);

            EditorSceneManager.SaveScene(scene, $"Assets/Scenes/{def.Scene}.unity");
            AddSceneToBuild($"Assets/Scenes/{def.Scene}.unity");
        }

        /// <summary>T014: replace placeholder rects with a 3D cave mesh background (low-poly, URP Lit).</summary>
        static void BuildCave3DBackground(CaveDef def)
        {
            string meshPath = $"Assets/Art/Caves/CaveBg_{def.MeshName}.asset";
            AssetDatabase.Refresh(); // ensure mesh generated by CaveBackgroundGenerator is visible
            var mesh = AssetDatabase.LoadAssetAtPath<Mesh>(meshPath);
            if (mesh == null)
            {
                Debug.LogWarning($"[CaveBg] Mesh not found at {meshPath} — run Hermes/Generate Cave Backgrounds first.");
                return;
            }

            var bg = new GameObject("Cave3DBg");
            bg.transform.position = new Vector3(0, 0, 20); // far back, behind all sprites
            var mf = bg.AddComponent<MeshFilter>();
            mf.sharedMesh = mesh;
            var mr = bg.AddComponent<MeshRenderer>();

            // URP Lit material with cave tint — unlit pass for flat low-poly look
            var mat = new Material(Shader.Find("Universal Render Pipeline/Lit"));
            mat.color = def.Wall;
            mat.SetFloat("_Smoothness", 0.1f);
            mr.sharedMaterial = mat;

            Debug.Log($"[CaveBg] 3D mesh {def.MeshName} → {mesh.triangles.Length/3} tris, tint={def.Wall}");
        }
